...
describe('Email class', () => {
  it('should have correct message for the rejected incidents', () => {
    ...
    email = new Email(user, incident);
    expect(email.message).to.equal(`Hello ${user.firstname}, your report #${incident.id} has been rejected because we could not verify your claim.`);
  });
  it('should have correct message for the resolved incidents', () => {
    incident.status = Constants.INCIDENT_STATUS_RESOLVED;
    email = new Email(user, incident);
    expect(email.message).to.equal(`Hello ${user.firstname}, your report #${incident.id} has been resolved. You may confirm and give us feedback.`);
  });
  it('should have correct message for the incidents under investigation', () => {
    ...
    email = new Email(user, incident);
    expect(email.message).to.equal(`Hello ${user.firstname}, your report ${incident.id} is currently under investigation. You will continue to get feedback from us as we progress.`);
  });
});
